...
loveButton.addEventListener("click", () => {
  ...
  if (monkeyBox.classList.contains("show")) {
    setTimeout(() => {
      const monkeyRect = monkeyBox.getBoundingClientRect();
      const monkeyTop = monkeyRect.top + window.pageYOffset;
      const monkeyHeight = monkeyRect.height;
      const screenHeight = window.innerHeight;

      const targetPosition = monkeyTop + monkeyHeight / 2 - screenHeight / 2;
